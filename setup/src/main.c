...
_tWinMain(HINSTANCE instance, HINSTANCE not_used_1, LPTSTR not_used_2, int not_used_3) {
  ...
  while(exit == ID_APPLY) {
    if (RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("Software\\Z\\diff-ext"), 0, KEY_READ, &key) == ERROR_SUCCESS) {
      ...
      GetModuleFileName(instance, language_lib, sizeof(language_lib)/sizeof(language_lib[0]));
...
init(HWND dialog, WPARAM not_used_1, LPARAM not_used_2) {
  ...
  if(file != INVALID_HANDLE_VALUE) {
    ...
    while(stop == FALSE) {
      ...
      result = _stscanf(file_info.cFileName, TEXT("diff-ext%4u.dll"), &lang_id);
